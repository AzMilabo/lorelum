...
describe('isSoftwareRenderer', () => {
  test('flags Windows software fallback (Microsoft Basic Render Driver)', () => {
    expect(
      isSoftwareRenderer('ANGLE (Microsoft, Microsoft Basic Render Driver Direct3D11)'),
    ).toBe(true);
  });
  ...
  test('does not flag real hardware GPUs', () => {
    expect(isSoftwareRenderer('ANGLE (NVIDIA, NVIDIA GeForce RTX 3050 Laptop GPU Direct3D11)')).toBe(
      false,
    );
