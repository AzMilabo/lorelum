...
import { decodeMarkdownUrl, encodeMarkdownUrl } from './shared';
...
describe('encodeMarkdownUrl', () => {
  test('appends .md to the last slug under /docs', () => {
    ...
    expect(encodeMarkdownUrl(['practices', 'verification'])).toBe('/docs/practices/verification.md');
  });
...
describe('decodeMarkdownUrl', () => {
  test('strips .md from the last segment', () => {
    ...
    expect(decodeMarkdownUrl(['practices', 'verification.md'])).toEqual(['practices', 'verification']);
  });
