...
- **Strict serial, in-revision-order delivery.** The hook is invoked serially per LocalStore instance, in monotonically increasing `effectiveRevision` order, never concurrently. A revision's delta is delivered at most once per process; a missed revision (process crash between commit and hook) is recovered by `lore reindex`, which re-derives and re-emits. The vector layer may rely on receiving a contiguous, ordered delta stream — it must not have to de-dupe or reorder. _How the v1 implementation satisfies this without an explicit queue:_ mutations are serialized by the cross-process mutation lock (§12), each mutation emits its hook exactly once (no emit when the Effective Practice set did not advance), and the hook is awaited before the mutation returns — so per-process delivery is inherently serial, monotonic, and at-most-once. The only gap is a crash between commit and hook, which §8's journal + `reindex` re-emit closes by design; an in-process revision queue is therefore **not** required and not implemented.
