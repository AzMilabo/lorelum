...
- Cloudflare 后台项目类型是 **Worker**（不是 Pages）；生产域名 `lorelum.com` 在后台作为 Worker 的自定义域绑定，Workers 默认的 `*.workers.dev` 域名仅用于测试
...
手动直传会直接更新**生产**（`lorelum.com`），所以适合在站点本就是展示/测试用途时使用；若是正式站，改版预览请走本地（第 3 步）或临时 preview 版本。
