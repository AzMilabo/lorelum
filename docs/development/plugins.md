...
for working on the current host Plugin artifacts (Codex, Cursor, WorkBuddy,
and ZCode) from a checkout; it does not define their public identities or
marketplace schemas.
...
| Cursor | [`plugins/cursor/lorelum`](../../plugins/cursor/lorelum) | Configuration tests |
...
```sh
bun test plugins/scripts plugins/codex/lorelum/scripts plugins/cursor/lorelum/scripts plugins/workbuddy/lorelum/scripts plugins/zcode/lorelum/scripts
...
printf '%s\n' '{"hook_event_name":"sessionStart"}' \
  | bun packages/cli/src/main.ts hook cursor --store-root /absolute/path/to/isolated-store
...
`.codebuddy-plugin/marketplace.json`. The public install path and its
desktop-registry caveats are documented in
[the WorkBuddy guide](https://lorelum.com/en/docs/workbuddy); for local
iteration, add the repository root as a marketplace with the embedded CodeBuddy
CLI:
...
`marketplace add` expects the repository root (it discovers the metadata
directory inside); passing `.codebuddy-plugin` itself or the manifest file
path is rejected. Fully quit and restart WorkBuddy after installing so the
resident CLI host reloads the registry.
